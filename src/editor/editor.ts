...
export function initEditor(): void {
  ...
  function rebuild(): void {
      ...
      if (n.dry && n.waterY !== undefined) {
        ...
        if (rel > -1 && rel < 1) {
          const disc = new THREE.Mesh(
            new THREE.CircleGeometry(Math.max(n.radius * s[0], n.radius * s[2]) * Math.sqrt(1 - rel * rel), 20),
            new THREE.MeshBasicMaterial({ color: 0x2a6a80, transparent: true, opacity: 0.5, side: THREE.DoubleSide }),
          );
    ...
    EDGES.forEach((e, index) => {
      ...
      const line = new THREE.Line(
        ...
        new THREE.LineBasicMaterial({ color, linewidth: 1, transparent: true, opacity: selected ? 1 : 0.85 }),
      );
      edgeGroup.add(line);
  ...
  renderer.domElement.addEventListener('dblclick', (e) => {
    pointer.set((e.clientX / window.innerWidth) * 2 - 1, -(e.clientY / window.innerHeight) * 2 + 1);
    raycaster.setFromCamera(pointer, camera);
    const hit = raycaster.intersectObjects(edgeGroup.children, false).find((h) => h.object.userData.edgeIndex !== undefined);
    if (!hit) return;
    const index = hit.object.userData.edgeIndex as number;
    const segIndex = hit.object.userData.segIndex as number;
    const e2 = EDGES[index];
    e2.waypoints = e2.waypoints ?? [];
    e2.waypoints.splice(segIndex, 0, [hit.point.x, hit.point.y, hit.point.z]);
    commit();
