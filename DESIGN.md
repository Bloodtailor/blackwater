...
2. **Air is the clock.** The anti-crawler-camping mechanic is environmental: below the surface you are always spending air, so there is no safe way to stall. Chores (box hits, perk runs, battery runs) happen under time pressure by physics, not by artificial timers.
...
| **Timed intermissions + frenzy rule** (§9): next round starts on a timer wherever you are; last zombies enrage if you stall | Removes the last stalling loophole breath doesn't cover (sitting at the surface between rounds). |
...
| **No door buys** | Original brief: passages never close — so nothing to unlock. Distance, darkness, and air are the gates. The door-money-sink role moves to consumables (batteries, reels, chemlights) and steep perk costs. |
...
- The intended winning arc: **power by round 4–6 → 2–3 perks by round 8–10 → Pack-a-Punch by round 12–15 → Heart run at round 15–20.** Go too early: the Abyss kills you. Go too late: rounds crush the ascent. The player's economy/routing decisions determine where in that window they land — that's the whole game.
...
- **Two-route rule:** between the surface and each zone hub there are ≥2 fully disjoint routes. A silt-out can always be swum *around*, never only waited out. Dead ends hang off the routes; they never gate progress.
...
- Mouse-look; WASD relative to look direction; Space/C for pure up/down; Shift sprint. Full freedom to swim any direction, including inverted.
...
### 6.3 Health
- 100 HP. Drowned grab: 35. Guardian hit: 70. BO1-style regen: after 5 s without damage, full over ~4 s. Down at 0 → death (run over) unless Second Wind (§10.4).
...
- Batteries: wall buy (250) and a guaranteed drop from Anglers; restore to 100%. Powered string lights (§10.3) cover main arteries so flashlight becomes a choice on known routes, a necessity off them.
...
- **Frenzy rule (anti-stall):** if ≤15% of the round's zombies remain for >45 s, they enrage — ×1.8 speed, direct pathing. There is no keepable crawler.
...
### 10.4 Perks — pick 4 of 9, stations fixed in data, power required
| Perk | Cost | Effect |
|---|---|---|
...
### 10.5 Pack-a-Punch — 5000, the Abyss
...
### 10.6 Drops (from kills, ~2% + pity timer)
...
- Grabbing it starts the **Ascent**: continuous fast spawning everywhere, all zombies at frenzy speed, global light stir (visibility down one grade), Guardians pursue beyond the Abyss. The Heart glows in your hands — you can see, and everything can see you. Weapons still usable (Heart stows on fire, glow persists).
...
Menus: title (Dive / How to Dive / Settings), pause, death screen with stats, win screen with stats. Settings: mouse sensitivity, invert Y, FOV, **max-tilt slider**, brightness, master/music/SFX/VO volume, subtitles (radio logs).
...
- Everything procedural (no modeling tools available): displaced tube/blob geometry from the cave graph, triplanar-noise rock shader, instanced particles (silt, bubbles, motes), billboard god-rays only at the sinkhole shaft.
...
| Decisions matter / no crawler-stalling / no pointless high rounds | Pillars 1–2, §4, §9 frenzy + timed intermissions |
