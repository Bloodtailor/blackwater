...
- **Guide line reel (controls rework 2026-07-19 round 12 — constant use, in a panic, zero combat-key overlap):** start with one 200 m reel. Two keys own the whole system, both clear of the shooter set (R stays reload, F flashlight/melee, LMB/RMB weapons, C swim-down). **T is the line hand** — *tap*: lay / stop / resume by context; starting auto-anchors to the nearest rock within 2 m instantly (the 4 s ceremony is gone; no rock in reach = the line trails from your hand); *hold*: ride the line (follow) — direction latches on engage, glide hand-over-hand at 3.5 m/s at any visibility with free look. **X is the spool hand** — *tap while laying*: instant tie-off; *hold near the free end* (or mid-lay — "wrong way"): reel in, an active glide toward the anchor that collects the line as you go; release leaves the rest stopped in place, recover the anchor and it's stowed. **Ctrl grabs the wall** (pure movement brace — no line duties). The HUD kit line always teaches the current options in place (e.g. `LINE END · T resume · hold X reel`). Extra reel: wall buy 750 (max 2 deployed / 400 m). Zombies do not cut lines (v1).
