...
- **Ambient current (user, 2026-07-18, strengthened ×3):** everywhere below the surface, a wandering current pushes the player hard enough to *fight or ride* — peak ~1.2 m/s vs 4 m/s swim, direction and strength shifting quickly but smoothly. It moves your *position*, never your camera; stop paying attention and you drift. Honest-tells guard: bubbles and gauge stay truthful.
- **Streamline momentum (user, 2026-07-18):** holding one direction builds speed toward sprint speed even without sprinting (~2 s of sprint or ~4 s without); changing direction dumps the built speed; sprint's value is faster buildup + holding it (plus the lunge burst). Feel is silky/icy — long glide, soft acceleration. Enough momentum breaches the surface in a dolphin arc (~1.5 m; airborne thrust is cut so it's an arc, not flight).
