...
Run intro: **Boone's letter** (LORE.md §2.3) as a skippable styled text card — it is how the player learns the objective (bottom of the bore, take the Heart, surface) plus two tutorial seeds, delivered in-world. Posters/blueprint/photos support **inspect** (look + E → fullscreen overlay; LORE.md §7 readability rules). Tapes: collected on pickup, auto-play at the next safe surfacing (no enemy within ~20 m), replayable from pause (LORE.md §5).
