...
|---|---|---|---|
| **The Sinkhole** (surface) | 0–17 m | Spawn. Open cenote: daylight shaft, **bare dry rock shore** (the camp props — table, cot, crates, lamp, kit — were **CUT 2026-08-02 by the user; do not re-dress the shore**. Only the hung print and the T1 tape recorder stand there), calm pool. Infinite air with head above water. | 2 wall buys (Speargun 500, Pneu-Driver 1000), 3 burrows. Burrows activate progressively; from round 5+ zombies climb into the pool itself — the surface stops being safe. |
...
### 12.1 The Concept Gallery (NEW 2026-07-21 — celebration system)

**❌ CUT 2026-08-02 (user): the Museum Annex and the morale button are REMOVED — room, exhibits, party, and all. Do not rebuild them.** The safe museum room off gal-rec (`src/game/annex.ts`, the `annex` layout node, the `museum` node flag, the `party` music source, `museum.1`/`rem.museum.1`) is deleted; the cave no longer has a true safe zone. The pause-menu **PHOTOGRAPHS** collection stays — prints the player inspects still file there. `public/music/morale-night.mp3` and the C12 concept painting are orphaned assets, kept on disk, referenced by nothing.

The Concept Gallery exists to *celebrate the story*, has **zero gameplay impact**, and will eventually be an unlockable. **v1 ships it unlocked by default** (the gate design below is written down now so a later session flips one switch):
...
- ~~**The party track:** its own generated song for the Museum Annex morale button (§12.1).~~ **❌ CUT 2026-08-02 (user) with the Annex.** `public/music/morale-night.mp3` stays on disk, wired to nothing; the `party` music source is gone from the MusicDirector.
...
**Debug-only means debug-only (user 2026-08-02):** N/noclip needs the harness live — `?debug`, or the panel on screen via F1. Leaving noclip always works, so no one is stranded flying when the panel closes. Hotkeys that are also *game* keys (F flashlight, G chemlight, R reload, V knife) stay unconditional. **Panel SAVE ⇒ disk must stick:** `src/tuning.overrides.json` is a JSON module the game imports and the panel rewrites at runtime — the dev server invalidates that module on every write and on hand edits (without reloading the page), and the panel remembers what it wrote, so consecutive saves in one tab accumulate instead of overwriting each other.
